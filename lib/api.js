...
export async function fetchSeriesData(slug) {
  const url = `${baseUrl}/track/season/${slug}`;
...
export async function getAudio(id) {
  const url = `${baseUrl}/track/audio/${id}`;
...
export async function getTopTrendingSongs() {
  const url = `${baseUrl}/track/trending/audios`;

  const response = await fetch(url, {
    next: {
      revalidate: 60 * 60,
      tags: ["topTrendingSongs"],
    },
  });
  // const response = await fetch(url, { cache: "no-store" });
  if (!response.ok) {
...
export async function getHeroSection() {
  const url = `${baseUrl}/hero-section`;
  const response = await fetch(url, {
    next: {
      revalidate: 60 * 60,
      tags: ["heroSection"],
...
export async function getBreakingNews() {
  const url = `${baseUrl}/hero-section/breaking-news`;
  const response = await fetch(url, {
    next: {
      revalidate: 60 * 60,
      tags: ["heroSection"],
...
export async function getTrendingMovies() {
  const url = `${baseUrl}/track/trending/movies`;
  //const response = await fetch(url, { cache: "no-store" });
  const response = await fetch(url, {
    next: {
      revalidate: 60 * 60,
