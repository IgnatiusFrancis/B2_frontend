...
const TrendingMusics = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await action("topTrendingSongs");
      toast.success("Latest Musics updated");
      router.push("/admin");
    } catch (error) {
      console.error("Upload error:", error);

      toast.error(
        error?.response?.data?.message || "Failed to update latest musics"
      );
    } finally {
      setUploading(false);
    }
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      ...
      try {
        const response = await axios.get(`${baseUrl}/track/audios`);
        const fetchedVideos = response?.data?.data.audios || [];

        setVideos(fetchedVideos);
        setFilteredVideos(fetchedVideos);
      } catch (error) {
        console.error("Unable to fetch music:", error);
      ...
      <div className="w-full max-w-4xl bg-white rounded-2xl shadow-2xl overflow-hidden">
        <div className="bg-gradient-to-r from-blue-500 to-purple-600 p-6 text-white">
          <h1 className="text-3xl font-bold tracking-tight">
            Latest Music Management
          </h1>
          <p className="text-blue-100 mt-2">
            Select and update your top trending Music
          </p>
        </div>
