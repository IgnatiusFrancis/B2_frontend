...
function TrendingVideos({ id, title, date, url, artist }) {
  ...
  return (
    <div className="group relative bg-white rounded-lg p-4 shadow-md hover:shadow-lg transition-all duration-300 hover:scale-105">
      {/* Artist Label/Badge */}
      {artist.isTopArtist && (
        <div className="absolute top-2 right-2 bg-primarycolor text-white text-xs px-3 py-1 rounded-full shadow-md">
          Trending
        </div>
      )}
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex flex-col justify-between">
          ...
          <p className="text-sm text-gray-600">{artist.name || "Unknown Artist"}</p>
